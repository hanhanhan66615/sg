...
/**
 * @brief       ATK-MW8266D连接MQTT服务器
 * @param       server_ip  : MQTT服务器IP地址
 *              server_port: MQTT服务器端口号
 * @retval      ATK_MW8266D_EOK  : 连接MQTT服务器成功
 *              ATK_MW8266D_ERROR: 连接MQTT服务器失败
 */
uint8_t atk_mw8266d_connect_mqtt_server(char *mqtt_ip, char *mqtt_port)
{
    uint8_t ret;
    char cmd[64];
    
	ret = atk_mw8266d_send_at_cmd("AT+MQTTUSERCFG=0,1,\"han\",\"ESP8266\",\"123456\",0,0,\"\"","OK",500);
	if (ret != ATK_MW8266D_EOK)
    {
        return ATK_MW8266D_ERROR;
    }
	sprintf(cmd,"AT+MQTTCONN=0,\"%s\",%s,0",mqtt_ip,mqtt_port);
...
uint8_t atk_mw8266d_mqtt_send(char *data)
{
	uint8_t ret;
	char cmd[64];
	
	sprintf(cmd,"AT+MQTTPUB=0,\"ESP8266/online\",\"%s\",0,0",data);
...
uint8_t atk_mw8266d_mqtt_publish(char *topic,uint8_t Qos)
{
	uint8_t ret;
	char cmd[64];
...
uint8_t atk_mw8266d_mqtt_clean(void)
{
	...
	ret = atk_mw8266d_send_at_cmd("AT+MQTTCLEAN=0","OK",500);
	if (ret == ATK_MW8266D_EOK)
    {
        return ATK_MW8266D_ERROR;
    }
